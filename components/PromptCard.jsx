    ...
    <div className="prompt_card">
      <div className="flex justify-between items-start gap-5">
        ...
        <div className="flex flex-col " >
          <h3 className="font-santoshi font-semibold cursor-pointer text-gray-900" onClick={handleProfileClick}>{post.creator.username}</h3>
          <p className="font-inter text-sm text-gray-500 cursor-pointer" onClick={handleProfileClick}>{post.creator.email} </p>
        </div>
        <div className="copy_btn" onClick={handleCopy}>
          <Image src={copied === post.prompt ? '/assets/icons/tick.svg' : '/assets/icons/copy.svg'} width={10} height={10} alt='copy'></Image>
        </div>
      </div>
